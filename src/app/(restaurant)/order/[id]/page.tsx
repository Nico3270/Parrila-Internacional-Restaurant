...
import { FaChair, FaClock, FaCheckCircle, FaUtensils, FaTimesCircle } from "react-icons/fa";
...
// Simulamos que obtenemos el estado de la orden de un servidor o estado global
enum OrderStatus {
  Creada = "Creada",
  Recibida = "Recibida",
...
const getStatusIconAndColor = (status: OrderStatus) => {
  ...
  switch (status) {
    ...
    case OrderStatus.NoPagada:
      return { icon: <FaTimesCircle className="text-red-500" />, color: "bg-red-100 text-red-600" };
    case OrderStatus.Pagada:
      return { icon: <FaCheckCircle className="text-green-700" />, color: "bg-green-400 text-green-600" };
    default:
...
export default function OrderDetail({params}:Props) {
  const {id} = params; 
  // Simulamos que obtenemos el tipo de entrega y estado desde un estado global
  const [isDelivery] = useState<DeliveryType>(DeliveryType.Domicilio);
  const [orderStatus] = useState<OrderStatus>(OrderStatus.Recibida);
  const orderId = id;
  ...
  const { icon, color } = getStatusIconAndColor(orderStatus);

  return (
    <div className="container mx-auto p-4">
      {/* Título dinámico de la orden */}
      <h1 className="text-4xl font-bold mb-8 text-center">Orden #{orderId}</h1>
      
      {/* Fecha de la orden */}
      <p className="text-center text-gray-500 mb-6">Fecha de la orden: {orderDate}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <p className="text-sm mt-2">
                  ...
                  {product.customizationOptions?.extras?.map((extra, i) => (
                    <span key={i}>
                      ...
                      {i < (product.customizationOptions?.extras?.length ?? 0) - 1 && ", "}
                    </span>
                  ))}
                </p>
        ...
        <div className="border rounded-lg shadow-md p-6 space-y-6">
          ...
          <div className={`flex items-center gap-3 p-4 border-2 rounded-lg ${color}`}>
            {icon}
            <span className="font-bold text-lg">{orderStatus}</span>
          </div>
